<?php
	...
	if(!isset($_GET['tag_name']) or strip_tags(trim($_GET['tag_name'])) == '0')
	{
		echo "<script>
	...
	if(isset($_GET['since']) && isset($_GET['until']) &&
					is_numeric($_GET['since']) && is_numeric($_GET['until']))
	{
		$since=strip_tags(trim($_GET['since']));
		$until=strip_tags(trim($_GET['until']));
		$day = round(($until - $since)/(60 * 60 * 24));
		$since_str=date('Y-m-d H:i:s', $since);
		$until_str=date('Y-m-d H:i:s', $until);
		$SQLChart = "SELECT [Tagname], CONVERT(char(10), [DateTime], 120)+' '+CONVERT(char(5), [DateTime], 108) AS [DateTime], ROUND([Value], 2) AS [Value]
		...
		if($conn)
		{
			...
			if($stmt === false) {
				...
				$current_row = 0;
				$iter_count = 0;
				//echo "$iter_count = DateTime,Значение<br>";
				...
				while($row = sqlsrv_fetch_array($stmt, SQLSRV_FETCH_ASSOC)){
					if(($current_row % ($num_rows / $maxCount)) == 0) {
						...
						$value = $row['Value'];
						$iter_count ++;
						//echo "$iter_count = $datetime,$value<br>";
						//echo "<pre>";
						echo "$datetime,$value<br>";
						//echo "</pre>";
					}
					$current_row += 1;
				}
